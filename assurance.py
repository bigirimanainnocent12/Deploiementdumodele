...
# Fonction pour charger le modèle avec gestion robuste des erreurs
@st.cache_resource
def load_model():
    ...
    try:
        # Essai 1 : Chargement standard avec joblib
        ...
        if hasattr(model, 'predict'):
            ...
            st.warning("⚠️ Modèle chargé mais sans méthode predict")
            return None
            
    except (AttributeError, ModuleNotFoundError, ImportError) as e:
        if any(keyword in str(e) for keyword in ['_RemainderColsList', 'sklearn', 'column_transformer']):
            st.warning("⚠️ Incompatibilité scikit-learn détectée")
            
            # Essai 2 : Chargement avec pickle standard
            try:
                import pickle
                with open(model_path, "rb") as file:
                    model = pickle.load(file)
                if hasattr(model, 'predict'):
                    st.info("✅ Modèle chargé avec pickle standard")
                    return model
            except Exception as pickle_error:
                st.write(f"Échec pickle standard: {pickle_error}")
            
            # Essai 3 : Chargement avec gestion des erreurs sklearn
            try:
                import sys
                import types
                from unittest.mock import MagicMock
                
                # Patch temporaire pour les attributs manquants
                if 'sklearn.compose._column_transformer' in sys.modules:
                    ...
                    if not hasattr(module, '_RemainderColsList'):
                        module._RemainderColsList = list
                
                with open(model_path, "rb") as file:
                    model = joblib.load(file)
                
                if hasattr(model, 'predict'):
                    st.info("✅ Modèle chargé avec patch sklearn")
                    return model
            except Exception as patch_error:
                st.write(f"Échec patch sklearn: {patch_error}")
            
            st.error("❌ Toutes les méthodes de chargement ont échoué")
            st.info("💡 Utilisation du mode simulation")
            return None
        else:
            st.error(f"Erreur inattendue lors du chargement : {str(e)}")
            return None
    
    except Exception as e:
        st.error(f"Erreur générale lors du chargement : {str(e)}")
        return None
...
def main():
    ...
    if st.checkbox("🔍 Diagnostic détaillé"):
        import sys
        try:
            import sklearn
            ...
            if os.path.exists('modele.pkl'):
                st.write(f"**Fichier modele.pkl:** Existe ({os.path.getsize('modele.pkl')} bytes)")
            else:
                st.write("**Fichier modele.pkl:** N'existe pas")
        except Exception as e:
